...
class WC_Gateway_Klarna_Invoice extends WC_Gateway_Klarna {
	...
	function process_payment( $order_id ) {
		...
		try {
			...
			$woocommerce->add_error( sprintf(__('Klarna payment failed (Correlation ID: %s). Payment was rejected due to an error: ', 'klarna'), urlencode($e->getMessage()) ) . '"' . $e->getCode() . '"' );
			// $woocommerce->add_error( 'Fel: ' . $e->getMessage() . ' Kod: ' . $e->getCode() );
