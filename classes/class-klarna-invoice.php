...
class WC_Gateway_Klarna_Invoice extends WC_Gateway_Klarna {
	...
	public function __construct() {
		...
		add_action( 'update_option_woocommerce_' . $this->id . '_settings', array(
			$this,
			'flush_pclasses_on_settings_save'
		), 10, 2 );
	}
	...
	function check_klarna_pending_callback( $order_id ) {
		...
		$order  = wc_get_order( $order_id );
		if ( $result == Klarna\XMLRPC\Flags::ACCEPTED ) {
		...
		} elseif ( $result == Klarna\XMLRPC\Flags::DENIED ) {
